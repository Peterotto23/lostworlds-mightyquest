...
struct LocalGameserver : Gameserver
{
    ...
    virtual void Send(
        const size_t Socket,
        std::string &Databuffer) override
    {
        ISSLServer::Send(
            Socket == 0 ? Current : Socket,
            Databuffer);
...
static void DrainTo(SOCKET cs, size_t sid)
{
    ...
    for (;;)
    {
        ...
        if (!g_gs->onStreamread(sid, out.data(), &len))
            break;
        int sent = 0;
        while (sent < (int)len)
        {
            int s = send(cs, out.data() + sent, (int)len - sent, 0);
            if (s <= 0)
                return;
            sent += s;
        }
...
static void HandleConnection(SOCKET cs)
{
    ...
    try
    {
        ...
        {
            std::lock_guard<std::mutex> lk(g_lock);
            g_gs->onDisconnect(sid);
        }
    }
    catch (const std::exception &e)
    {
        LOG("connection %u dropped (error: %s)", (unsigned)sid, e.what());
    }
    catch (...)
    {
        LOG("connection %u dropped (unknown fault)", (unsigned)sid);
    }
    closesocket(cs);
}
...
int main(int argc, char **argv)
{
    ...
    if (argc > 1)
        port = (unsigned short)atoi(argv[1]); // optional: mqelserver.exe <port>

    LOG("MQEL standalone server starting (Lost Worlds project)");
    _mkdir("Plugins");
    _mkdir("Plugins\\Logs"); // MQELOffline persists data/logs here
